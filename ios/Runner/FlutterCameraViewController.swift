    ...
    override open func viewDidLoad() {
        super.viewDidLoad()
             let backButton = UIButton(type: .system)
                     backButton.setImage(UIImage(systemName: "arrow.left"), for: .normal) // Set back button image
                     backButton.tintColor = .white // Customize button color
                     backButton.addTarget(self, action: #selector(backButtonPressed), for: .touchUpInside) // Set button action
                     view.addSubview(backButton) // Add button to view hierarchy
