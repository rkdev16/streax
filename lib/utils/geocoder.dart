...
import 'package:streax/utils/fetch_geocoder.dart';
...
class Geocoder {
  ...
  static Future<GeoData> getDataFromCoordinates({
   ...
   debugPrint("Url = $url");
    ...
    if (response.statusCode == 200) {
      ...
      // if(addressComponent.isNotEmpty){
      //   var address = addressComponent.first;
      ...
      //   }
      //   if (address.types.contains("postal_code")) {
      //     postalCode = address.longName;
      //   }
      //   if (address.types.contains("administrative_area_level_1")) {
      //     state = address.longName;
      //   }
      //   if (address.types.contains("street_number")) {
      //     streetNumber = address.longName;
      //   }
      // }
      ...
      for (var i = 0; i < addressComponent.length; i++) {
        ...
        if(addressComponent[i].types.contains("administrative_area_level_3")){
          city = addressComponent[i].longName;
        }
  ...
  ///Get City ,country , postalCode,state,streetNumber and countryCode from address like "277 Bedford Ave, Brooklyn, NY 11211, USA"
  static Future<GeoData> getDataFromPlaceId({
    required String placeId,
    required String googleMapApiKey,
    String? language,
  }) async {
    //  var url = language != null ? 'https://maps.googleapis.com/maps/api/geocode/json?address=$address&key=$googleMapApiKey&language=$language' : 'https://maps.googleapis.com/maps/api/geocode/json?address=$address&key=$googleMapApiKey';
    var url = language != null ? 'https://maps.googleapis.com/maps/api/geocode/json?place_id=$placeId&key=$googleMapApiKey&language=$language' : 'https://maps.googleapis.com/maps/api/geocode/json?place_id=$placeId&key=$googleMapApiKey';
    var request = http.Request('GET', Uri.parse(url));
